...
from .Locations import Overcooked2Location, si_location_name_to_id, si_location_id_to_name
...
class SpiritIslandWorld(World):
    ...
    def get_priority_locations(self) -> List[int]:
        ...
        target_progression_ratio = (
            game_progression_density - total_progression_density) / 2.0 + total_progression_density
        # I'm sorry I round like an old person
        target_progression_count = int(
            (target_progression_ratio * game_item_count) + 0.5)
    ...
    def generate_early(self):
        ...
        self.enabled_dlc = {Overcooked2Dlc(x)
                            for x in self.options.include_dlcs.value}
        ...
        self.level_unlock_counts = level_unlock_requirement_factory(
            self.options.stars_to_win.value)
        ...
        if self.options.shuffle_level_order:
            ...
            if Overcooked2Dlc.STORY not in self.enabled_dlc:
                raise Exception(
                    f"Invalid OC2 settings({self.player_name}) Need either Level Shuffle disabled or 'Story' DLC enabled")
    ...
    def set_location_priority(self) -> None:
        ...
        for level in Overcooked2Level():
            if level.level_id in priority_locations:
                location: Location = self.multiworld.get_location(
                    level.location_name_item, self.player)
                location.progress_type = LocationProgressType.PRIORITY
        ...
        for level in Overcooked2Level():
            ...
            level_access_rule: Callable[[CollectionState], bool] = \
                ...
                has_requirements_for_level_access(
                    state, level_name, previous_level_completed_event_name, required_star_count, self.options.ramp_tricks.result, self.player)
            self.connect_regions(
                "Overworld", level.level_name, level_access_rule)
    ...
    def create_items(self):
        ...
        for item_name in item_table:
            ...
            while freq > 0:
                ...
                # only the first progressive item can be progression
                classification = ItemClassification.useful
                freq -= 1
        ...
        pool_count = len(si_location_name_to_id)
        if not self.options.kevin_levels:
            pool_count -= 8

        while len(self.itempool) < pool_count:
            self.itempool.append(self.create_item(
                "Bonus Star", ItemClassification.useful))
    ...
    def place_events(self):
        ...
        for level in Overcooked2Level():
            ...
            if level.level_id != 36:
                self.place_event(level.location_name_level_complete,
                                 level.event_name_level_complete)
    ...
    def fill_json_data(self) -> Dict[str, Any]:
        ...
        for level_id in self.level_unlock_counts:
            level_purchase_requirements[str(
                level_id)] = self.level_unlock_counts[level_id]
        ...
        if self.options.kevin_levels:
            def kevin_level_to_keyholder_level_id(level_id: int) -> Optional[int]:
                location = self.multiworld.find_item(
                    f"Kevin-{level_id-36}", self.player)
                if location.player != self.player:
                    return None  # This kevin level will be unlocked by the server at runtime
                level_id = si_location_name_to_id[location.name]
                return level_id

            for level_id in range(37, 45):
                keyholder_level_id = kevin_level_to_keyholder_level_id(
                    level_id)
                if keyholder_level_id is not None:
                    level_unlock_requirements[str(
                        level_id)] = keyholder_level_id
        ...
        for location in locations:
            ...
            if location.item.player != self.player:
                continue  # not for us
            level_id = str(si_location_name_to_id[location.name])
            on_level_completed[level_id] = [
                item_to_unlock_event(location.item.name)]
        ...
        bugs = ["FixDoubleServing", "FixSinkBug",
                "FixControlStickThrowBug", "FixEmptyBurnerThrow"]
        for bug in bugs:
    ...
    def write_spoiler(self, spoiler_handle: TextIO) -> None:
        ...
        world: SpiritIslandWorld = self
        spoiler_handle.write(f"\n\n{self.player_name}'s Level Order:\n\n")
        for overworld_id in world.level_mapping:
            overworld_name = Overcooked2GenericLevel(
                overworld_id).shortname.split("Story ")[1]
